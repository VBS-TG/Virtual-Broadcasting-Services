#!/usr/bin/env bash
set -euo pipefail

if [[ -z "${VBS_EYEVINN_OPENLIVE_BASE_URL:-}" ]]; then
  echo "錯誤: 需設定 VBS_EYEVINN_OPENLIVE_BASE_URL（Eyevinn Open Live 控制端點；可使用 https://vbsapi.cyblisswisdom.org 並由路徑分流）" >&2
  exit 1
fi

if [[ "${VBS_ENGINE_TELEMETRY_ENABLED:-1}" != "0" ]]; then
  if [[ -z "${VBS_CF_ACCESS_AUD:-}" ]]; then
    echo "錯誤: 啟用 telemetry 時需設定 VBS_CF_ACCESS_AUD" >&2
    exit 1
  fi
  if [[ -z "${VBS_CF_ACCESS_CLIENT_ID:-}" || -z "${VBS_CF_ACCESS_CLIENT_SECRET:-}" ]]; then
    echo "錯誤: 啟用 telemetry 時需同時設定 VBS_CF_ACCESS_CLIENT_ID 與 VBS_CF_ACCESS_CLIENT_SECRET" >&2
    exit 1
  fi
fi

if [[ "${VBS_ENGINE_CONTROL_API_ENABLED:-1}" != "0" ]]; then
  if [[ -z "${VBS_CF_ACCESS_AUD:-}" || -z "${VBS_CF_ACCESS_CLIENT_ID:-}" || -z "${VBS_CF_ACCESS_CLIENT_SECRET:-}" ]]; then
    echo "錯誤: 啟用控制面驗證時需設定 VBS_CF_ACCESS_AUD、VBS_CF_ACCESS_CLIENT_ID、VBS_CF_ACCESS_CLIENT_SECRET" >&2
    exit 1
  fi
fi

echo "[vbs-engine] 啟動 Eyevinn Open Live adapter..."
exec node /opt/vbs-engine/dist/index.js

