...
def main() -> None:
    ...
    new = (
        ...
        "        if name is None:\n"
        "            name = factory_name\n"
        "        base_name = who_its_for.uid + '_' + name + '_' + str(random.randint(1, 1000000))\n"
        "        input_bin = getattr(self, 'final_' + audio_or_video + '_tee').parent\n"
        "\n"
        "        def _new_element(suffix=''):\n"
        "            candidate = base_name + suffix\n"
        "            if input_bin.get_by_name(candidate) or self.pipeline.get_by_name(candidate):\n"
        "                candidate = candidate + '_' + str(random.randint(1, 1000000))\n"
        ...
        "        e_retry = e if e is not None else _new_element('_retry')\n"
        "        if e_retry and _safe_add(input_bin, e_retry):\n"
        "            self.pipeline.set_state(prev_state)\n"
        "            return e_retry\n"
        "        # Recreate with a fresh name to avoid duplicate-name collisions\n"
        "        e_retry2 = _new_element('_retry2')\n"
        "        if e_retry2 and _safe_add(input_bin, e_retry2):\n"
        "            self.pipeline.set_state(prev_state)\n"
        "            return e_retry2\n"
        "        self.pipeline.set_state(prev_state)\n"
        "\n"
        "        e_pipe = _new_element('_pipe')\n"
